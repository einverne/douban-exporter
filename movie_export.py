...
class MovieInfo:
    ...
    def parse(cls, item):
        instance = cls()
        instance.title = item.select('.title a')[0].text.strip().encode("utf-8")
        instance.url = item.select('.title a')[0]['href']
        instance.intro = item.select('.intro')[0].text.strip().encode("utf-8")
        ...
        if len(item.select('.tags')) > 0:
            instance.tags = item.select('.tags')[0].text.encode("utf-8")
        if len(item.select('.comment')) > 0:
            instance.comment = item.select('.comment')[0].text.strip().encode("utf-8")
        return instance
